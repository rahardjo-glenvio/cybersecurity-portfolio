...
import JAGADIntro from './JAGADIntro';
import JAGADSpread from './JAGADSpread';
import './JAGAD.css';

// ─── Reusable avatar with photo + fallback ────────────────────────────────────
function JAGADAvatar({ size = 26 }) {
  const [imgOk, setImgOk] = useState(true);
  return (
    <div className="jagad-avatar" style={{ width: size, height: size, flexShrink: 0 }}>
      {imgOk ? (
        <img
          src="/jagad-face.png"
          alt="JAGAD"
          className="jagad-avatar-img"
          onError={() => setImgOk(false)}
        />
      ) : (
        <span className="jagad-avatar-fallback">AI</span>
      )}
    </div>
  );
}
...
function HBar({ label, value, max, color }) {
  ...
  return (
    <div className="jagad-hbar">
      <span className="jagad-hbar-label">{label}</span>
      <div className="jagad-hbar-track">
        <div className="jagad-hbar-fill"
          style={{ width: `${pct}%`, background: color, boxShadow: `0 0 8px ${color}80` }} />
      </div>
      <span className="jagad-hbar-val">{value}</span>
    </div>
  );
}
...
function buildAnalysis(alerts) {
  ...
  const brief = [
    `JAGAD telah menganalisis **${alerts.length} security event** dari **${uniqueIPs} unique IP address** dalam 24 jam terakhir. Risk Score sistem: **${riskScore}/100**.`,
    riskSentence,
    critSentence,
    svcSentence,
    geoSentence,
...
function buildResponse(cmd, analysis, alerts) {
  ...
  if (cmdLower.includes('brief') || cmdLower.includes('ringkas') || cmdLower.includes('analisis') || cmdLower.includes('summary') || cmdLower.includes('laporan singkat')) {
    return [
      { type:'text', text:`Berikut laporan analisis ancaman komprehensif berdasarkan **${total} event** yang diproses JAGAD:` },
...
function ResponseItem({ item, onSend }) {
  ...
  if (item.type === 'capabilities') return (
    <div className="jagad-capabilities">
      {CAPABILITY_LIST.map((cap, i) => (
        <button key={i} className="jagad-cap-item" onClick={() => onSend?.(cap.cmd)}>
          <span className="jagad-cap-icon">{cap.icon}</span>
          <div className="jagad-cap-body">
            <div className="jagad-cap-label">{cap.label}</div>
            <div className="jagad-cap-desc">{cap.desc}</div>
          </div>
          <span className="jagad-cap-arrow">›</span>
        </button>
      ))}
    </div>
  );
  if (item.type === 'risk') return (
    <div className="jagad-data-card">
      <div className="jagad-data-title">RISK SCORE</div>
      <div className="jagad-risk-row">
        <RiskGauge score={item.score} />
        <div className="jagad-risk-info">
          <div className="jagad-risk-label" style={{ color: item.score>=70?'#ff0044':item.score>=45?'#ff8800':item.score>=20?'#ffdd00':'#00ff88' }}>
            {item.score}/100
          </div>
          <div className="jagad-risk-sub">{item.crit} CRITICAL · {item.total} TOTAL</div>
        </div>
      </div>
    </div>
  );
  if (item.type === 'attacks') return (
    <div className="jagad-data-card">
      <div className="jagad-data-title">DISTRIBUSI SERANGAN</div>
      <div className="jagad-chart-row">
        <DonutChart data={item.serviceData} size={110} centerLabel="ATTACKS" />
        <div className="jagad-legend">
          {item.serviceData.map(d => (
            <div key={d.label} className="jagad-leg-item">
              <span className="jagad-leg-dot" style={{ background: d.color }} />
              <span className="jagad-leg-label">{d.label}</span>
              <span className="jagad-leg-count">{d.value}</span>
              <span className="jagad-leg-pct">{Math.round(d.value/item.total*100)}%</span>
            </div>
          ))}
        </div>
      </div>
  ...
  if (item.type === 'severity') return (
    <div className="jagad-data-card">
      <div className="jagad-data-title">TINGKAT KEPARAHAN</div>
      <div className="jagad-chart-row">
  ...
  if (item.type === 'geo') return (
    <div className="jagad-data-card">
      <div className="jagad-data-title">TOP NEGARA PENYERANG</div>
  ...
  if (item.type === 'mitre') return (
    <div className="jagad-data-card">
      <div className="jagad-data-title">MITRE ATT&CK TECHNIQUES</div>
  ...
  if (item.type === 'brief') {
    ...
    return (
      <div className="jagad-data-card">
        <div className="jagad-data-title">AI THREAT BRIEF</div>
        {paras.map((para, i) => (
          <p key={i} className="jagad-brief-text"
            dangerouslySetInnerHTML={{ __html: para.replace(/\n/g, '<br>') }} />
        ))}
      </div>
    );
  ...
  if (item.type === 'recs') return (
    <div className="jagad-data-card">
      <div className="jagad-data-title">REKOMENDASI</div>
      <ul className="jagad-recs">
        {item.recs.map((r,i) => (
          <li key={i}><span className="jagad-rec-num">0{i+1}</span>{r}</li>
        ))}
      </ul>
    </div>
  );
  if (item.type === 'timeline') return (
    <div className="jagad-data-card">
      <div className="jagad-data-title">ATTACK TIMELINE</div>
      <div className="jagad-timeline">
        {item.alerts.map((a,i) => {
          ...
          return (
            <div key={i} className="jagad-tl-item">
              <div className="jagad-tl-dot" style={{ background:col, boxShadow:`0 0 5px ${col}` }} />
              <div className="jagad-tl-body">
                <div className="jagad-tl-title">
                  <span style={{color:col,fontWeight:'bold'}}>[{sevLabel}]</span>
                  {' '}<span style={{color:'rgba(0,200,240,0.8)'}}>{a.service}</span>
                  {' '}· {a.source_ip}
                </div>
                <div className="jagad-tl-desc">{a.rule_description||'Unknown event'}</div>
                <div className="jagad-tl-meta">
                  📍 {a.source_city || '?'}, {a.source_country || '?'}
                  {mitre ? ` · 🎯 ${mitre}` : ''}
                  {` · Rule ${a.rule_id||'?'}`}
                </div>
              </div>
              <div className="jagad-tl-time">
                <div>{time}</div>
...
// ─── Main JAGAD Component ─────────────────────────────────────────────────────
export default function JAGAD({ alerts, onClose, closing }) {
  // intro: JAGADIntro visible, panel hidden | done: panel slides in
  ...
  const sendMessage = (cmd) => {
    ...
    // Simulate JAGAD "thinking" then respond
    setTimeout(() => {
      const responseItems = buildResponse(text, analysis, alerts || []);
      setIsTyping(false);
      setMessages(prev => [...prev, { from:'jagad', items: responseItems }]);
    }, 600 + Math.random() * 400);
  };
  ...
  return (
    <>
      ...
      {phase !== 'intro' && <div className={`jagad-panel${closing ? ' jagad-panel-closing' : ''}`}>
      ...
      {/* Header */}
      <div className="jagad-header">
        <div className="jagad-branding">
          <div>
            <JAGADAvatar size={34} />
          </div>
          <div>
            <div className="jagad-name">JAGAD</div>
            <div className="jagad-fullname">Jaringan Analisis Garda Ancaman Digital</div>
          </div>
        </div>
        <button className="jagad-close-btn" onClick={onClose}>✕</button>
      </div>

      {/* Status */}
      <div className="jagad-status-bar">
        <div className="jagad-status-dot" />
        <span>ONLINE</span>
      ...
      {/* Chat area */}
      <div className="jagad-chat-area" ref={chatRef}>
        {messages.map((msg, i) => (
          msg.from === 'user' ? (
            ...
            <div key={i} className="jagad-msg">
              <JAGADAvatar size={26} />
              <div className="jagad-msg-body">
        ...
        {isTyping && (
          <div className="jagad-typing">
            <JAGADAvatar size={26} />
            <div className="jagad-typing-dots">
              <span /><span /><span />
            </div>
          </div>
        )}
      ...
      {/* Quick chips */}
      <div className="jagad-chips-wrap">
        {CHIPS.map(c => (
          <button key={c.cmd} className="jagad-chip"
      ...
      {/* Input bar */}
      <div className="jagad-input-bar">
        <input
          className="jagad-input"
          value={input}
          onChange={e => setInput(e.target.value)}
          onKeyDown={handleKey}
          placeholder="Tanya JAGAD sesuatu..."
          disabled={isTyping}
        />
        <button className="jagad-send-btn" onClick={() => sendMessage()} disabled={!input.trim() || isTyping}>
          ▸
        </button>
      </div>
    </div>}
